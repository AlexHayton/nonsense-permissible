...
test.describe('Homepage Tests', () => {
  ...
  test('should display hero section', async ({ page }) => {
    ...
    // Check hero title
    await expect(hero.locator('h1')).toContainText('Crafting Experiences That Spark Joy');
    
    // Check hero description
    await expect(hero.locator('p')).toContainText('At Nonsense Permissible, we create web, mobile and XR experiences');
    
    // Check hero buttons
    await expect(hero.locator('button:has-text("View Our Work")')).toBeVisible();
  ...
  test('should display featured work section', async ({ page }) => {
    ...
    const projectCards = workSection.locator('div[class*="group"]').filter({ hasText: 'View Project' });
    await expect(projectCards).toHaveCount(3);
    
    // Check specific projects
    await expect(workSection.locator('h3:has-text("ValleyDAO Phlo")')).toBeVisible();
    await expect(workSection.locator('h3:has-text("Quest Alarm Clock")')).toBeVisible();
    await expect(workSection.locator('h3:has-text("Video Dice")')).toBeVisible();
  });
  ...
  test('should have external links in footer', async ({ page }) => {
    ...
    const githubLink = page.locator('footer a[href="https://github.com/alexhayton"]');
    await expect(githubLink).toBeVisible();
    await expect(githubLink).toHaveAttribute('href', 'https://github.com/alexhayton');
    
    const linkedinLink = page.locator('footer a[href="https://www.linkedin.com/in/alexhayton/"]');
    await expect(linkedinLink).toBeVisible();
    await expect(linkedinLink).toHaveAttribute('href', 'https://www.linkedin.com/in/alexhayton/');
  });
  ...
  test('should open mobile menu', async ({ page }) => {
    ...
    await expect(mobileMenuDropdown.locator('button:has-text("Get in Touch")')).toBeVisible();
  });
